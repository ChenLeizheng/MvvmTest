package com.landleaf.everyday.wheelview;
...
public class WheelView extends View implements IWheelViewSetting {
    ...
    private void drawItem(Canvas canvas, Rect rect, IWheel item, int offsetY, TextPaint textPaint) {
        ...
        Log.d("WheelView", "item.getType():" + item.getType()+","+startX+",getWidth"+getWidth()+"w:"+w);
        if (item.getType() == DateItem.TYPE_HOUR){
            startX = getWidth() - 2*w;
        }
        if (item.getType() == DateItem.TYPE_MINUTE){
            startX = w;
        }
        Log.d("WheelView", "startX:" + item.getType()+","+startX);
        canvas.save();
