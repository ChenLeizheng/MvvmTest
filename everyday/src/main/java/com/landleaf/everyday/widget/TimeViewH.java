...
public class TimeViewH extends RelativeLayout {

    private static final String TAG = TimeViewH.class.getSimpleName();
    ...
    public TimeViewH(Context context) {
        this(context,null);
    }

    public TimeViewH(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        if (!isInEditMode()) {
            // 造成错误的代码段
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.TimeViewH);
            ...
            //size
            int hourSize = typedArray.getDimensionPixelOffset(R.styleable.TimeViewH_hourSize, 34);
            int monthSize = typedArray.getDimensionPixelOffset(R.styleable.TimeViewH_monthSize, 24);
            int weekSize = typedArray.getDimensionPixelOffset(R.styleable.TimeViewH_weekSize, 22);
